...
class ItemEstoqueDao {
    ...
    total(callback) {
        const sql = `SELECT count(*) as count FROM estoque`;
    ...
    all(callback) {
        const sql = `SELECT estoque.id, itens.nome, itens.preco, estoque.quantidade_atual FROM estoque LEFT JOIN itens on estoque.id_item = itens.id`;
